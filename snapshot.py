...
while True:
    ...
    def deleteAllSnapshots(image):
        while True:
            if (len(client.images.get_all(type="snapshot")) == len(serverNames)*int(noOfSnapshotsKept)): break
            elif (len(client.images.get_all(type="snapshot")) < len(serverNames)*int(noOfSnapshotsKept)): return
        i=0
        for name in image:
            if(not name):
    ...
    scheduler.add_job(runSnapshot,'interval', hours=12)
    scheduler.start()
